create or replace view `{{ project_id_tgt }}.{{ dataset_reporting_tgt }}.InvoiceDocuments_Flow` as
WITH
      ...
      RSEG AS (
        SELECT
          * EXCEPT (wrbtr, bnkan),
          ...
          END AS bnkan
        FROM
          `{{ project_id_tgt }}.{{ dataset_cdc_processed }}.rseg`
      ),
      ...
      RBCO AS (
        SELECT
          * EXCEPT (wrbtr, bnkan_fw),
          ...
          END AS bnkan_fw
        FROM
          `{{ project_id_tgt }}.{{ dataset_cdc_processed }}.rbco`
      ),
    ...
    FROM RSEG_RBCO
    INNER JOIN `{{ project_id_tgt }}.{{ dataset_cdc_processed }}.rbkp` AS RBKP
      ON RSEG_RBCO.mandt = RBKP.mandt
        AND RSEG_RBCO.belnr = RBKP.belnr
        AND RSEG_RBCO.gjahr = RBKP.gjahr
    LEFT JOIN `{{ project_id_tgt }}.{{ dataset_cdc_processed }}.prps` AS PRPS
