create or replace view `{{ project_id_tgt }}.{{ dataset_reporting_tgt }}.AccountingDocuments` as
WITH BSEG AS (
        SELECT
          * EXCEPT (
            DMBTR,
          ...
          END AS PENFC
        FROM
          `{{ project_id_tgt }}.{{ dataset_cdc_processed }}.bseg`
      )
      ...
      FROM BSEG AS BSEG
      INNER JOIN `{{ project_id_tgt }}.{{ dataset_cdc_processed }}.bkpf` AS BKPF
      ...
      LEFT JOIN `{{ project_id_tgt }}.{{ k9_datasets_processing }}.calendar_date_dim` AS CalendarDateDimension_BUDAT
        ON CalendarDateDimension_BUDAT.Date = BKPF.BUDAT
      LEFT JOIN `{{ project_id_tgt }}.{{ k9_datasets_processing }}.calendar_date_dim` AS CalendarDateDimension_BLDAT
